...
=== Deploying your own version of `pfcon` 

If you want to deploy your own version of `pfcon` you can edit `pfcon-openshift-template.json` which is present in pfcon/openshift. You will find a line containing "image": "fnndsc/pfcon:pfiohless", which is basically the name of the docker image we want to deploy. As an example you can change it to cagriyoruk/pfcon. So it would look like "image": "cagriyoruk/pfcon". (See the image below)

...
2. Create `pfioh_config & pman_config`
a.. Create a file `example-config.cfg` and add the following:
...
b.. Convert the configuration to base64 & copy the base64 encoded results with the following command:

....
cat example-config.cfg | base64
....

c.. Create a file `example-secret.yml` and add the encoded result (For `pfioh`):
...
d.. Create the secret for `pfioh`

....
oc create -f example-secret.yml
....

e.. Edit the file example-secret.yml and add the following (For `pman`):
...
3. Create `swift-credentials`

a.. Create a file swift-credentials.cfg and add the following:
...
b.. Create the secret `swift-credentials`

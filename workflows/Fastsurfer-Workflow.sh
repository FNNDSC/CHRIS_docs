...
DESC
  ...
  engine, ``pl-fastsurfer_inference`` and related machinery to CUBE:
      ...
      ███                                    0:0   pl-mri_sag_anon_192
       ...
       │
          ↓
      ███                                    0:2   pl-pfdicom_tagsub
     ┌─┴─┐                                          (anonimize tags)
       ↓    │
    ███  │                                   2:3   pl-pfdicom_tagextract
         │                                          (extract substituted tags)
             ↓
        ███                                  2:4   pl-fshack
         │                                          (Run freesurfer [ARGS: -autorecon1] only)
         │
        ███                                  4:5   pl-fastsurfer_inference
         ┼───┬                                      (CNN Segmentation)
         │  ███                              5:6   pl-multipass
         │   │                                     (mgz2image)
         │   ↓
...
windowBottom
    ...
    plugin_run  ":1" "a_WORKFLOWSPEC[@]" "$CUBE" ID1 $sleepAfterPluginRun \
                "@prev_id=$ROOTID;@image='m:%_nospc|-_ProtocolName.jpg';@scale=3:none" && id_check $ID1
